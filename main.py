...
if __name__ == "__main__":
    ...
    # maybe pass as a paramter later
    MODEL = "gpt-4o-mini"
    ...
    if DEV_MODE:
        ...
        # Get git hash to track the running results of different versions
        git_hash = os.popen("git rev-parse HEAD").read().strip()
        run_dir = os.path.join(dir, f"run_{time.strftime('%Y%m%d')}_{MODEL}_{git_hash}_RAG")

    if not os.path.exists(run_dir):
        os.makedirs(run_dir)
    # I modified the create_state function in utils.py
    # The previous version directly load the parameter json
    # But the parameter json actually needs to be generated by LLM first 
